...
public class GlobalBMLM {
    ...
    public GlobalBMLM(HeuristicConfig hConfig){
    ...
    public TransparentNetwork allocate( TransparentNetwork network, IncrementalTraffic incTraffic, List<ModulationConfig> modulationList,
        ...
        List<Traffic> trafficList = new LinkedList<Traffic>();
        for( int matrix = 0; matrix < incTraffic.getTrafficMatrixList().size(); matrix++)
            trafficList.addAll( incTraffic.getTrafficMatrixList().get(matrix).getTrafficList() );
        ...
        for(int tf = 0; tf < trafficMatrix.getTrafficList().size(); tf++ ){
            ...
            // Choosing the best path, which has the smallest maximum occupied slot index
    ...
    public void setSortingEnabled(boolean isSortingEnabled) {
        this.sortingEnabled = isSortingEnabled;
    }
